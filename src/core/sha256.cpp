...
// Straightforward FIPS 180-4 SHA-256, single-shot (no incremental API --
// nothing here needs to hash a stream).

namespace vb::core {

namespace {

constexpr std::array<std::uint32_t, 64> kK = { 0x428a2f98, 0x71374491,
	...
	for (std::size_t chunk = 0; chunk < msg.size(); chunk += 64) {
		...
		for (int i = 0; i < 16; ++i) {
			...
			w[static_cast<std::size_t>(i)] =
					(static_cast<std::uint32_t>(msg[o]) << 24) |
		...
		for (int i = 0; i < 64; ++i) {
			...
			const std::uint32_t temp1 = hh + s1 + ch +
					kK[static_cast<std::size_t>(i)] + w[static_cast<std::size_t>(i)];
	...
	for (int i = 0; i < 8; ++i) {
		for (int j = 0; j < 4; ++j) {
			const std::uint8_t byte = static_cast<std::uint8_t>(
					h[static_cast<std::size_t>(i)] >> ((3 - j) * 8));
			out[static_cast<std::size_t>(i * 8 + j * 2)] = kHex[byte >> 4];
			out[static_cast<std::size_t>(i * 8 + j * 2 + 1)] = kHex[byte & 0xF];
		}
	}
	return out;
}
